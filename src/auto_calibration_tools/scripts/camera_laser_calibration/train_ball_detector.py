...
class SyntheticDataset(Dataset):
    def __init__(self, target_image, backgrounds, num_samples, device, augmentation_probability=1.0, use_advanced_aug=True):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < self.augmentation_probability and self.adv_aug:
            target_distorted = self.apply_perspective_distortion(target_resized)
        else:
            target_distorted = target_resized
...
def main():
    ...
    target_image_path = os.path.join(root,'target_ball.png')  # The image with transparency
    output_folder = 'synthetic_data/'
    test_image_folder = os.path.join(root, 'images_UHD_ball_indoor2/')

    # Load the target image (with transparency)
    target_image = Image.open(target_image_path).convert('RGBA')
    # List comprehension to load only files, not folders
    backgrounds = [os.path.join(backgrounds_folder, bg) for bg in os.listdir(backgrounds_folder) if
                   os.path.isfile(os.path.join(backgrounds_folder, bg))]
    ...
    save_path = os.path.join(root, 'one_shot_ball_detector_SIDES.pth')
