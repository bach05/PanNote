...
class Plotter:
    ...
    def save(self, side, img_id):

        path = os.path.join(side, "log_imgs")
    ...
    def processImage(self, image_path, verbose=True):
        ...
        if side is not None:
            ...
            if len(box_with_corners) == 2:
                ...
                return [side, left_x_points, right_x_points]
...
class LaserPointFinder:
    ...
    def processScan(self, ranges, verbose=True):
        ...
        #atanB = np.arctan2(B[1], B[0])
        #atanC = np.arctan2(C[1], C[0])

        # if (atanB * atanC > 0) or ((abs(atanB) + abs(atanC)) < np.pi):
        #     if atanB > atanC:
        #         X = B
        #         B = C
        #         C = X
        ...
        # if (atanB > 0 and atanC > 0):
        #     if atanB > atanC:
        #             X = B
        #             B = C
        #             C = X
        # elif (atanB < 0 and atanC < 0):
        #     if atanB < atanC:
        #             X = B
        #             B = C
        #             C = X
        ...
        #             B = C
        #             C = X
        # elif  ((abs(atanB) + abs(atanC)) < np.pi) and (atanB > atanC):
        #             X = B
        #             B = C
        #             C = X
        # elif  ((abs(atanB) + abs(atanC)) > np.pi) and (atanB < atanC):
...
def main():
    ...
    image_folder = './images_UHD_indoor'
            ...
            if img_results is not None:
                ...
                if laser_results is not None:
                    ...
                    if verbose:
                        ...
                        plotter.save(side[1], os.path.splitext(bag_name)[0])

                    #DATA ASSOCIATION AND SAVE: left image point = C, right image points = B
                    data = ((C, left),(B, right), image_filename)
