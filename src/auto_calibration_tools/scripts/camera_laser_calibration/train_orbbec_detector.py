...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < self.augmentation_probability:
            scale_x = random.uniform(0.5, 1.25)
            scale_y = random.uniform(0.5, 1.25)
        else:
            scale_x = scale_y = 1.0
...
def main():
    ...
    # Define paths
    root = args.folder
    backgrounds_folder = os.path.join(root,'backgrounds_orbbec')
    target_image_path = os.path.join(root,'targetL_orbbec.png')  # The image with transparency
    target2_image_path = os.path.join(root,'targetR_orbbec.png')  # The image with transparency
    output_folder = 'synthetic_data/'
    test_image_folder = os.path.join(root, 'images_orbbec/')
    ...
    save_path = os.path.join(root, 'one_shot_object_detector_orbbec.pth')
