            ...
            with rosbag.Bag(self.path, 'r') as bag:
                ...
                try:
                    for i, (laser_msg, image_msg) in tqdm(enumerate(zip(laser_iterator, image_iterator))):
                        ...
                        if i == 0:
                            time_start = min(laser_time, image_time)
                        ...
                        while abs(laser_time - image_time) > 0.04:  # Adjust the threshold as needed
                            if laser_time < image_time:
                                ...
                                plt.scatter(laser_times_np, np.ones_like(laser_times_np) + 1, color="blue",
                                            label="laser")
                                plt.scatter(laser_times_np[laser_mask_np],
                                            np.ones_like(laser_times_np[laser_mask_np]) + 1, color="cyan",
                                            label="laser")

                                for k, j in zip(img_times_np, laser_times_np[~laser_mask_np]):
                                    plt.plot([k, j], [1, 2], "--y")
                        ...
                        with rosbag.Bag(self.path, 'r') as bag:
                            ...
                            try:
                                for i, (laser_msg, image_msg) in tqdm(enumerate(zip(laser_iterator, image_iterator))):
                                    ...
                                    # Calculate the angles for each range measurement
                                    angles = np.arange(laser_spec['angle_min'], laser_spec['angle_max'],
                                                       laser_spec['angle_increment'])
                            ...
                            plt.scatter(img_times, np.ones_like(img_times), color="red", label="image")
                            plt.scatter(laser_times, np.ones_like(laser_times) + 1, color="blue", label="laser")
                            plt.scatter(laser_times[laser_mask], np.ones_like(laser_times[laser_mask]) + 1,
                                        color="cyan", label="laser")
